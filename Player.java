...
public abstract class Player {
   ...
   public void move(char dir){
        int[] newPos;
        newPos = Map.playerMovement(x, y, dir, tile);
        setX(newPos[0]);
        setY(newPos[1]);
   }
   ...
   //place player function which is used at the beginning of a game to set initial position of both players
    public void placePlayer() {
        ...
        int maxY = Map.getMapY();
        int maxX = Map.getMapX();
        int chosenX = 0;
        int chosenY = 0;
        ...
        while (!placed) {
            ...
            boolean onGold = Map.checkPlayerOnGold(chosenX, chosenY);
            //check if on gold
            boolean inWall = Map.checkPlayerInWall(chosenX, chosenY);
            //check if in wall
            if (!onGold && !inWall) {
                placed = true;
            }
